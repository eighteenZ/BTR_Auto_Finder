...
def test_create_campaign_ignores_generation_self_approval(monkeypatch, tmp_path):
    """Regression: under approval mode, auto_send_eligible set by the generation
    stage (model self-approval) must NOT make a sequence campaign-ready —
    only an explicit human decision counts."""
    app = create_app()
    client = TestClient(app)
    ...
    res = client.post("/api/v1/hunts/hunt_1/email-campaigns", json={"name": "Self Approval Rejected"})
    assert res.status_code == 200
    assert res.json()["sequence_count"] == 0
