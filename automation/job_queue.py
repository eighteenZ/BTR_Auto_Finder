"""Persistent queue for headless hunt jobs (PostgreSQL via SQLAlchemy)."""
...
class HuntJobQueue:
    def __init__(self, db_path: str = "") -> None:
        ...
        # the queue now lives in the configured PostgreSQL database.
    ...
    def count_by_status(self, *statuses: str) -> int:
        ...
        with get_session() as session:
            row = fetch_one(
                session,
                f"SELECT COUNT(*) AS count FROM hunt_jobs WHERE status IN ({placeholders})",
                list(statuses),
            )
    ...
    def count_finished_since(self, status: str, since_iso: str) -> int:
        with get_session() as session:
            row = fetch_one(
                session,
                "SELECT COUNT(*) AS count FROM hunt_jobs WHERE status = ? AND finished_at >= ?",
                (status, since_iso),
            )
        return int(row["count"]) if row else 0
    ...
    def count_retrying_since(self, since_iso: str) -> int:
        with get_session() as session:
            row = fetch_one(
                session,
                """
                SELECT COUNT(*) AS count FROM hunt_jobs
    ...
    def get(self, job_id: str) -> dict[str, Any] | None:
        with get_session() as session:
            row = fetch_one(session, "SELECT * FROM hunt_jobs WHERE id = ?", (job_id,))
        return self._hydrate(row) if row else None

    def is_cancellation_requested(self, job_id: str) -> bool:
        with get_session() as session:
            row = fetch_one(session, "SELECT status, progress_stage FROM hunt_jobs WHERE id = ?", (job_id,))
        if not row:
            return False
        return str(row["status"] or "") == "failed" and str(row["progress_stage"] or "") == "cancelled"
    ...
    def update_progress(
        ...
        *,
        ...
        with get_session() as session:
            execute(session, f"UPDATE hunt_jobs SET {', '.join(fields)} WHERE id = ?", values)
