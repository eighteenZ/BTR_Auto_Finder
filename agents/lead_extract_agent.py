...
def _candidate_domain(result: dict) -> str:
    """Best-effort official domain for a search result row."""
    domain = _official_website_domain(result.get("link", ""))
    if domain:
        return domain
    maps = result.get("maps_data") or {}
    return _official_website_domain(result.get("website") or maps.get("website") or "")


def _enough_contacts(lead: dict, minimum: int) -> bool:
    emails = [e for e in (lead.get("emails") or []) if str(e).strip()]
    return len(emails) >= max(0, int(minimum))
...
def _lead_identity(lead: dict) -> str:
    ...
    for email in lead.get("emails") or []:
        cleaned = str(email).replace("(inferred)", "").strip().lower()
        if cleaned:
            return f"e:{cleaned}"
    return ""


def _merge_lead_lists(*groups: list[dict]) -> list[dict]:
    """Concatenate lead lists, dropping duplicates by domain/email identity."""
    merged: list[dict] = []
    seen: set[str] = set()
    for group in groups:
        for lead in group:
...
async def lead_extract_node(
    state: HuntState,
    *,
    lead_lookup: Callable[[list[str]], dict[str, dict]] | None = None,
    dedup_mode: str | None = None,
    reuse_enrichment: bool | None = None,
    reuse_min_contacts: int | None = None,
) -> dict:
    ...
    Global dedup / reuse: when ``lead_lookup`` is provided, already-known
    leads (by official domain) are reused instead of re-scraped.
    ...
    resolved_dedup_mode = dedup_mode or str(state.get("dedup_mode", "") or "") or settings.lead_dedup_mode
    resolved_reuse_enrichment = (
        settings.lead_reuse_enrichment if reuse_enrichment is None else bool(reuse_enrichment)
    )
    resolved_min_contacts = (
        settings.lead_reuse_min_contacts if reuse_min_contacts is None else int(reuse_min_contacts)
    )

    # ── Prefetch known leads for global dedup / reuse ─────────────────────
    known_by_domain: dict[str, dict] = {}
    if lead_lookup is not None and resolved_dedup_mode in ("reuse", "skip") and resolved_reuse_enrichment:
        candidate_domains = sorted(
            {d for d in (_candidate_domain(r) for r in search_results if isinstance(r, dict)) if d}
        )
        if candidate_domains:
            try:
                known_by_domain = await asyncio.to_thread(lead_lookup, candidate_domains) or {}
            except Exception as exc:  # noqa: BLE001
                logger.warning("[LeadExtractAgent] Known-lead lookup failed, continuing fresh: %s", exc)
                known_by_domain = {}

    reused_leads: list[dict] = []
    reused_domains: set[str] = set()

    if target_lead_count and len(existing_leads) + len(reused_leads) >= target_lead_count:
        logger.info(
            "[LeadExtractAgent] Skipping extraction because existing leads already satisfy target (%d/%d)",
            len(existing_leads) + len(reused_leads),
            target_lead_count,
        )
        if resolved_dedup_mode == "skip":
            reused_leads = []
        return {
            "leads": _merge_lead_lists(existing_leads, reused_leads),
    ...
    for r in search_results:
        ...
        if not link and not maps_title:
            continue
        link_official_domain = _candidate_domain(r)
        ...
        if link_official_domain and link_official_domain in seen_candidate_domains:
            continue
        if (
            link_official_domain
            and link_official_domain in known_by_domain
            and link_official_domain not in reused_domains
            and _enough_contacts(known_by_domain[link_official_domain], resolved_min_contacts)
        ):
            reused_domains.add(link_official_domain)
            reused_leads.append({**known_by_domain[link_official_domain], "reused": True})
            continue
    ...
    logger.info(
        "[LeadExtractAgent] Reusing %d known lead(s) from repository (dedup_mode=%s)",
        len(reused_leads),
        resolved_dedup_mode,
    )
    ...
    return {
        "leads": _merge_lead_lists(existing_leads, reused_leads, new_leads),
        "keyword_search_stats": keyword_stats,
        "current_stage": "lead_extract",
    }
