...
class HuntRequest(BaseModel):
    ...
    dedup_mode: Literal["reuse", "skip", "off"] | None = Field(
        default=None, description="Global lead dedup behavior; None uses the configured default"
    )
...
class ResumeRequest(BaseModel):
    ...
    template_seed: dict[str, Any] | None = None
    dedup_mode: Literal["reuse", "skip", "off"] | None = None
...
def _build_hunt_graph(dedup_mode: str | None):
    """Build the pipeline graph with global lead reuse/dedup wired in."""
    node = functools.partial(
        lead_extract_node,
        lead_lookup=lead_repo.find_by_domains,
...
async def _persist_hunt_leads(hunt_id: str, accumulated: dict[str, Any], dedup_mode: str | None) -> None:
    """Merge/persist discovered leads into the repository and normalize the result."""
    ...
    try:
        canonical = await run_db(
            ...
            leads,
            hunt_id=hunt_id,
            dedup_mode=dedup_mode or get_settings().lead_dedup_mode,
        )
...
async def _run_hunt(hunt_id: str, request: HuntRequest) -> None:
    ...
    logger.info(
        "[Hunt %s] Starting — url=%s, files=%d, desc=%r, keywords=%s, profile=%s, regions=%s, email=%s, dedup=%s",
    ...
    try:
        ...
        graph = _build_hunt_graph(request.dedup_mode)
...
def _slim_state(prior_result: dict, request: ResumeRequest) -> dict:
    ...
    return {
        ...
        "dedup_mode": request.dedup_mode or prior_result.get("dedup_mode", ""),
